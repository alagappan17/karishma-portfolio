...
- A click-expandable experience timeline with recognition and related work.
...
- **Do** preserve factual content in `src/data/site.json` and `src/data/experience.json`, keep each project’s facts and named case-study sections together in `src/content/projects/*.md`, and map experience `projectSlugs` to project slugs instead of duplicating link data.
